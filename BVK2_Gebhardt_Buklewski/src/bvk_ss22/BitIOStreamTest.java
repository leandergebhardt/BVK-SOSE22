...
public class BitIOStreamTest {
	...
	private void testBitOutputStream(int num) throws Exception {
		...
		assertEquals("Writing " + num + " values: OutputStream size mismatch:", streams[num-1].length, bytes.length);
		
		for(int i = 0; i < streams[num-1].length; i++) {
			assertEquals("Writing " + num + " values: Value " + i + " mismatch:", streams[num-1][i], ((int)bytes[i]) & 0xff);
		}
	...
	private void testBitInputStream(int num) throws Exception {
		...
		for(int i = 0; i < num; i++) {
			assertEquals("Reading " + num + " values: Value " + i + " mismatch:", values[i], readValues[i]);
		}
